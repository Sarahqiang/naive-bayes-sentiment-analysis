...
import numpy as np
from dataclean import testtfidf
from dataclean import traintfidf
from math import e
import itertools
...
class MultinomialNaiveBayes:
    ...
    def fit(self, x, y , alpha):
        ...
        for c in range(C):
            x_c = x[y == c]
            Nc[c] = x_c.shape[0]
            mu[c, :] = np.sum(x_c,axis=0)
            sigma[c, :] = 1e-9 + np.std(x_c, 0)
...
# model.fit(traintfidf,sentiment, 1)
# y_prob = model.predict(testtfidf)
# y_pred = np.argmax(y_prob, 1)
